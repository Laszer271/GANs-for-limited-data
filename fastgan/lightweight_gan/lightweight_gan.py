...
class AugWrapper(nn.Module):
    ...
    def forward(self, images, prob = 0., types = [], detach = False, **kwargs):
        ...
        with context():
            ...
            for t in types:
                if random() < prob:
                    images = DiffAugment(images, types=[t])
...
GENERATOR_ARGS = ('image_size', 'latent_dim', 'fmap_max',
                  'fmap_inverse_coef', 'transparent',
                  'greyscale', 'attn_res_layers', 'freq_chan_attn')
...
DISCRIMINATOR_ARGS = ('image_size', 'fmap_max', 'fmap_inverse_coef',
                      'transparent', 'greyscale', 'disc_output_size',
                      'attn_res_layers')
...
GENERAL_ARGS = ('lr', 'optimizer', 'rank')
        ...
        **kwargs,
        ...
        for key in kwargs:
            ...
            if key in GENERATOR_ARGS:
                ...
                key_valid = True
            if key in DISCRIMINATOR_ARGS:
                D_kwargs[key] = kwargs[key]
                key_valid = True
            if key in GENERAL_ARGS:
                general_kwargs[key] = kwargs[key]
                key_valid = True
            elif not key_valid:
                raise AttributeError(f'keyword {key} was found neither in GENERATOR_ARGS nor in DISCRIMINATOR_ARGS')
        ...
        try:
            with open(config_file, 'r') as f:
                ...
                for key in config['general']:
                    print(f'{key} found in config for general')
                    if key in general_kwargs:
                        print(f"value changed from {general_kwargs[key]} to {config['general'][key]}")
                    else:
                        print(f"value {config['general'][key]} loaded")
                    general_kwargs[key] = config['general'][key]
                
                self.latent_dim = general_kwargs['latent_dim']
                G_kwargs['latent_dim'] = self.latent_dim

                self.image_size = general_kwargs['image_size']
                G_kwargs['image_size'] = self.image_size
                D_kwargs['image_size'] = self.image_size
                
                for key in config['generator']:
                    print(f'{key} found in config for generator')
                    if key in G_kwargs:
                        print(f"value changed from {G_kwargs[key]} to {config['generator'][key]}")
                    else:
                        print(f"value {config['generator'][key]} loaded")
                    G_kwargs[key] = config['generator'][key]
                    
                for key in config['discriminator']:
                    print(f'{key} found in config for discriminator')
                    if key in D_kwargs:
                        print(f"value changed from {D_kwargs[key]} to {config['discriminator'][key]}")
                    else:
                        print(f"value {config['discriminator'][key]} loaded")
                    D_kwargs[key] = config['discriminator'][key]
                    
        except FileNotFoundError:
            print(f'No config file {config_file} was found')
            print('Proceeding without config')


        print('Generator parameters:', G_kwargs)
        print('Discriminator parameters:', D_kwargs)
        print('General parameters:', general_kwargs)
        self.G = Generator(**G_kwargs)
        self.D = Discriminator(**D_kwargs)
        ...
        opt_cnt = 0
        if 'optimizer' in general_kwargs:
            optimizer_gen = general_kwargs['optimizer']
            optimizer_disc = general_kwargs['optimizer']
            opt_cnt = 2
        if 'optimizer' in G_kwargs:
            optimizer_gen = G_kwargs['optimizer']
            opt_cnt += 1
        if 'optimizer' in D_kwargs:
            optimizer_disc = D_kwargs['optimizer'] 
            opt_cnt += 1
        if  opt_cnt < 2:
            raise AttributeError('optimizer parameter should be given either in kwargs or in config')

        lr_cnt =  0
        if 'lr' in general_kwargs:
            lr_gen = general_kwargs['lr']
            lr_disc = general_kwargs['lr']
            lr_cnt = 2
        if 'lr' in G_kwargs:
            lr_gen = G_kwargs['lr']
            lr_cnt += 1
        if 'lr' in D_kwargs:
            ...
            lr_cnt += 1
        if  opt_cnt < 2:
            raise AttributeError('lr parameter should be given either in kwargs or in config')

        if optimizer_gen == "adam":
            self.G_opt = Adam(self.G.parameters(), lr=lr_gen, betas=(0.5, 0.9))
        elif optimizer_gen == "adabelief":
            self.G_opt = AdaBelief(self.G.parameters(), lr=lr_gen, betas=(0.5, 0.9))
        else:
            raise AttributeError("No valid optimizer is given")
            
        if optimizer_disc == 'adam':
            self.D_opt = Adam(self.D.parameters(), lr=lr_disc, betas=(0.5, 0.9))
        elif optimizer_disc == "adabelief":
            self.D_opt = AdaBelief(self.D.parameters(), lr=lr_disc, betas=(0.5, 0.9))
        else:
            raise AttributeError("No valid optimizer is given")
        ...
        self.cuda(general_kwargs['rank'])
        self.D_aug = AugWrapper(self.D, self.image_size)
