...
from stylegan2_ada.torch_utils.ops import conv2d_gradfix
from stylegan2_ada.torch_utils.ops import grid_sample_gradfix
...
def save_image_grid(img, name, drange, grid_size):
    lo, hi = drange
    ...
    assert C in [1, 3]
    if C == 1:
        img = PIL.Image.fromarray(img[:, :, 0], 'L')#.save(fname)
    if C == 3:
...
def training_loop(
    ...
    print('Constructing resolution:', training_set.resolution)
    ...
    if rank == 0:
        ...
        save_image_grid(images, 'Reals', drange=[0,255], grid_size=grid_size)
        grid_z = torch.randn([labels.shape[0], G.z_dim], device=device).split(batch_gpu)
        grid_c = torch.from_numpy(labels).to(device).split(batch_gpu)
        images = torch.cat([G_ema(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()
        save_image_grid(images, 'FakesInit', drange=[-1,1], grid_size=grid_size)
    ...
    while True:
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            images = torch.cat([G_ema(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()
            save_image_grid(images, 'FakesGenerated', drange=[-1,1], grid_size=grid_size)
